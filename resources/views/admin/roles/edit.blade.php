...
@section('content')
    <div class="mb-8">
        <div class="flex items-center gap-2 text-[12px] text-[--text-muted] mb-2">
            ...
            <span class="font-medium text-[--text-main]">Editar Rol</span>
        </div>
        <h1 class="text-[24px] font-semibold text-[--text-main]">Editar Rol</h1>
        <p class="text-[14px] text-[--text-muted]">Actualice las capacidades del perfil <b>{{ $role->name }}</b></p>
    </div>

    <div class="max-w-2xl">
        <form action="{{ route('admin.roles.update', $role->id) }}" method="POST" class="card space-y-6">
            @csrf
            @method('PUT')
            
            <div class="grid grid-cols-2 gap-6">
                <div>
                    <label class="block text-[12px] font-bold text-[--text-main] uppercase tracking-wide mb-2">Nombre del Rol</label>
                ...
                <div>
                    <label class="block text-[12px] font-bold text-[--text-main] uppercase tracking-wide mb-2">Identificador (Slug)</label>
            ...
            <div class="pt-6 border-t border-[--border-soft]">
                <label class="block text-[12px] font-bold text-[--text-main] uppercase tracking-wide mb-4">Capacidades asignadas (Permisos)</label>
                <div class="grid grid-cols-1 gap-3">
                    @foreach($permissions as $permission)
                        @php $hasPermission = $role->permissions->contains($permission->id); @endphp
                        <label class="flex items-center gap-3 p-3 rounded-lg border border-[--border-soft] cursor-pointer hover:bg-gray-50/50 transition {{ $hasPermission ? 'bg-blue-50/30 border-blue-100' : '' }}">
            ...
            <div class="pt-6 flex justify-end gap-3">
                <a href="{{ route('admin.roles.index') }}" class="px-6 py-2 text-[14px] font-bold text-[--text-muted] hover:bg-gray-100 transition rounded-lg">Cancelar</a>
